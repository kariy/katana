...
///
/// The values checked depend on the selected settlement mode (`tee`):
/// * StarknetOs mode — bootloader / SNOS-program / layout-bridge program hashes and the
///   `StarknetOsConfig3`-tagged config hash.
/// * KatanaTee mode — the `KatanaTeeConfig1`-tagged environment config hash.
///
/// In both modes, the fact-registry address is compared against `expected_fact_registry`.
///
...
fn build_program_info(tee: bool, chain_id: Felt) -> ProgramInfo {
    if tee {
        ...
        ProgramInfo::StarknetOs(StarknetOsProgramInfo {
            snos_config_hash: compute_config_hash(chain_id, DEFAULT_FEE_TOKEN_ADDRESS),
